...
def train_attacker():
    ...
    codes1_ = np.load('/data/UTAH_save/CSQ/ResNet50/CASIA/0.8828318460072873/database_code_jpeg50.npy')
    ...
    #             alpha=0.5, s=80)
    plt.scatter(centers[2:length_db+2, 0], centers[2:length_db+2, 1], color='#70B48F', label='Uncompressed', marker='.',
                alpha=0.5, s=50)
    plt.scatter(centers[length_db+2:2*length_db+2, 0], centers[length_db+2:2*length_db+2, 1], color='#5861AC',
                label='Compressed', marker='.', alpha=0.5, s=50)
    ...
    plt.savefig('./tsne/jpeg_clusters')
    plt.show()
